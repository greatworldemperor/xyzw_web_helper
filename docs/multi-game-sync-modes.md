...
| 全局同步 | `global` | 所有窗口（含未分组）视为一个大组，同步源 = **手动指定的窗口**（点窗口标题），没有手动指定时才回退到第一个分组的组长；其余窗口只接收 |
...
- 没有任何已打开窗口落在分组内时，**分组同步**不生效（不会拿「窗口顺序第一个」之类的东西兜底），**全局同步**则改用「手动指定的同步源」（见下）；
- 分组归属只在 Token 管理里编辑，批量页只维护「顺序 + 组长 + 模式 + 手动全局源」四项本地状态，避免同一份数据两处改。

## 全局同步源：点窗口标题手动指定

全局同步的同步源有两级：

1. **手动指定**（优先）：全局同步模式下，窗口卡片标题变成可点按钮（虚线下划线），点一下就把该窗口设为全局同步源，**再点一次取消**。它只按窗口记录，**完全不依赖 Token 管理分组** —— 临时拼起来的一批窗口，哪怕一个分组都没有、也没设过组长，也能直接选队长。
2. **回退默认**：没有手动指定（或指定的窗口已关闭）时，才用「第一个分组的组长」。

- 存储：`localStorage.multiGameSyncGlobalSource` = 窗口 scopeId（空字符串 = 未指定）。scopeId 每次多开会话生成，关掉那个窗口后自动作废并回退。
- 只在全局同步模式生效：分组同步下窗口标题是纯文本（各组的组长另有入口：同步栏下拉 / 卡片上的分组标签），手动源不会干扰分组同步。
- 同步栏摘要会标出来源：`● 全局：a 驱动 3 个窗口（手动指定）` / `（第一个分组的组长）`。
- 一个窗口都没有分组时，摘要与同步栏提示都会指向「点窗口标题指定同步源」这条通路，不再只报「未生效」。

## 分组与组长

- 分组归属 = Token 管理里的分组（`multiGameTokenGroups`，稳定键 `serverId:roleId`），批量页不复制这份数据。
- 分组**顺序**由批量页自己维护（`multiGameSyncGroupOrder`，分组 id 数组）：拖动同步栏里的 ⠿ 手柄，或用 chip 上的 ← / → 按钮调整。它决定「第一个分组」，也就是全局同步的**默认源**（手动指定过同步源时以手动为准）。
...
| 文件 | 职责 |
| --- | --- |
| `src/utils/multiGameSyncPlan.js` | 纯逻辑：模式归一化、分组排序、组长解析、全局源解析（`resolveGlobalSource`）、角色/targets 计算（单测 `test/multiGameSyncPlan.test.js`） |
...
| `local-data/_smoke_sync_source.mjs` | 运行时自查（零依赖：起 vite dev + headless Edge/CDP）：造 3 个**没有分组**的窗口，验证点标题改同步源、摘要文案、落盘、iframe 收到的 send / receive config |
