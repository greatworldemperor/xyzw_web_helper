...
- **战令档位奖励 —— 全量扫描 + 服务端裁定**（**不能用积分/公式筛选**，原因见 §5.9）：
  - 候选 = `complete > 0 && taskClaimed !== true` 全集（约 26 个），逐条发 `activity_warordertaskclaim`
  - 与游戏里「领取」按钮**行为等价**（它也是一档一条命令，按 missionId 升序）
  - 「是否真达标」交服务端裁定（未达标 `700010` / 已领 `700020`），失败按错误码汇总成一行
  - 代价约 13 秒/账号，换来**不漏领**；回归测试锁了「客户端实际会领的 4 个 ID 必须都在候选里」
...
### 5.9 档位机制：领取是「发 N 条命令」，档位 ID **不可本地推导**（2026-09-19 定稿）

master 给出规则与行为（**只能从 UI 效果层面确认，无法从后端确认，且他手上没有可测试的角色**）：

> 「积分每 1000 可以领取一档」「战令按钮是领取当前积分能抵达的挡位的全部，不是最后这一个，
> 也不是按顺序按一下领一个，而是把能领的全领了」

**协议侧已证实的部分**（不依赖任何推测）：抓包里那次「领取」是客户端**按 missionId 升序、一个档位发一条**
`activity_warordertaskclaim`，每条各自返回一份 `Activity_WarOrderClaimResp` 与对应奖励：

```
08:21:30 taskclaim {missionId:260919141} → 5282×400
08:21:30 taskclaim {missionId:260919147} → 5282×400
08:21:32 taskclaim {missionId:260919148} → 5282×400
08:21:34 taskclaim {missionId:260919149} → 5282×400
```

→ **「一次全领」在协议层就是 N 条独立命令**；本工具的实现与它**行为等价**（同样逐条发、同样按序）。
至于服务端是否另有聚合命令，**无法确认**（见下）。

**⚠️ 档位 ID 不能用公式推导**（这是上一版推断被推翻的地方）：

| 观察 | 结论 |
| --- | --- |
| 4 档时客户端实际发的是 `141 / 147 / 148 / 149` | **不连续**（141 直接跳到 147）→ `档N = actId+(46+N)` 之类的公式是**错的** |
| `role.items[5282]` 两账号都是 **3100**，按 1000/档只该 **3** 档 | 却各领了 **4** 个 → **5282 不是档位解锁口径**，不能用来算候选 |
| `complete[141/142/143] = 4000`（三个同值）而 `147~149 = 21`、`150~169 = 1` | `complete` 在各键段的语义不统一，**无法反推「哪一个是档位」** |

**因此实现只能走「全量扫描」**：候选 = `complete > 0 && taskClaimed !== true` 的全集（约 26 个），
逐条交给服务端裁定（未达标 `700010` / 已领 `700020`），失败按错误码汇总。
代价是每账号约 13 秒，换来的是**不漏领** —— 回归测试里专门锁了这条不变式：
**客户端实际会领的那 4 个 ID 必须全部落在候选里**。

**已尝试、走不通的取证路径**（不要再重复）：
- `src/xyzw/index.js`（仓库里的反混淆客户端，2.4MB）**是 7.7.12**，全文没有 `warorder` → 早于威震战令，无命令表
- 线上 `/game/index.html` 的 boot 脚本（`patch.decrypted_readable.js` / `main.*.js` / `xh.js`）都不含活动代码
  —— 真正的活动逻辑在动态加载的 Cocos 资源里，需要按需抓取 + 反混淆才可能拿到档位表
- 结论：**不依赖客户端代码**，用「全量扫描 + 服务端裁定」即可正确工作；要精确化只能靠**多档位样本抓包**
  （例如积分到 4 档时再抓一次，看客户端发的 ID 集合），但 master 目前**没有可测试的角色**，故暂不追求。
