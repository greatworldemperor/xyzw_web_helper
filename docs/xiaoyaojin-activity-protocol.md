...
- **战令等级奖励（档位奖励）**：
  - **主力：积分驱动**（每 1000 积分一档，见 §5.9）→ 只领 `floor(积分/1000)` 档里未领的，通常 1~3 次请求
  - **兜底**：读不到积分时退回 `complete > 0` 全量扫描（慢，日志会标 warning）
  - 只按 `taskClaimed` 判已领，「是否真达标」交服务端裁定（未达标 `700010` / 已领 `700020`），失败按错误码汇总
...
### 5.9 档位机制破解：积分 = 道具 5282，每 1000 分一档（master 口述 + 算术互证）

master 给出规则：**「积分每 1000 可以领取一档」**，截图右下角显示 **3100 分**、进度条 **`100/1000`**、
「所以领取了三次奖励」。
...
| 证据 | 内容 |
| --- | --- |
| 抓包 | 两份抓包结束时 `role.items[5282].quantity` **都恰好是 3100**（momo: 1500 +4×400；法王: 900 +4×400 +2×300） |
| 截图 | 右下角那个数就是 **3100**，图标与奖励格子里的**蓝卷轴**一致（5282 正是每日任务 +300 / 档位 +400 给的那个道具） |
| 算术 | `3100 = 3 × 1000 + 100` ↔ 截图「已领 3 档 + 第 4 档 `100/1000`」**精确吻合** |

**档位 → missionId 映射**（两个账号独立印证）：

```
档 N 的 missionId = actId + (46 + N)          → 档1 = 260919147、档2 = 148、档3 = 149、档4 = 150
```

- 两账号都恰好领了 **147 / 148 / 149 三个连续 ID** = **3 档** ↔ 积分 3100 ✓
- `150`（= 第 4 档）未被领取 ↔ 第 4 档还在 `100/1000` **未解锁** ✓
- `141` 是**另一类**奖励（`complete:4000`，与档位组的 21 / 1 完全不同量级；且 master 是**先领它**再按升序领档位）
- 档位组的值：`147~149 = 21`（已领）、`150~169 = 1`（未解锁）、`170~173 = 0`（未配置）→ 档位范围 **档1~档23**
- ⚠️ 积分**不在** `warOrderActivityInfo` 里（13 个字段全查过，`itemNum` 恒为 0）→ 只能从 `role.items[5282]` 读

**实现（`listIssuedPassTiers`）**：
- **主力路径（精确）**：读 `role.items[5282]` → 档数 = `floor(积分/1000)` → 只领「档 1..档数」里未领的
  → 通常 **1~3 次请求**（取代原来的 26 次盲试 ≈13 秒）
- **兜底路径**：读不到积分（字段缺失/请求失败）→ 退回 `complete > 0` 全量扫描，日志标注 `warning`
- 两条路径都只用 `taskClaimed` 判已领，**不购买任何东西**

⚠️ **映射是推断的、尚未现场验证**：两个账号当前都已领满 3 档（积分 3100），**没有可领的第 4 档来实测**
→ 等某账号积分涨到 4000 时，若 `260919150` 能被领取即确认；否则精确路径会「少领」而退回全量扫描才补齐。
因此**兜底路径必须保留**（宁可慢，不可漏）。
