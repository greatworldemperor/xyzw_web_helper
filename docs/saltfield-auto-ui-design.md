...
### 3.1.1 两种候选池（master 定稿 2026-09-16：编队不能依赖战场）

**提前编队发生在开战之前**，此时 `legion_getbattlefield` / `war_enterbattlefield` 都不可用 —— 所以「编辑弹窗加载成员」绝不能走战场探测。由此拆成两种池：

| | 名册池 `buildRosterCandidatePool` | 战场池 `buildCandidatePool` |
| --- | --- | --- |
| 数据源 | `legion_getinfo` 名册（**仅主连接**，开战前后随时可拉） | 名册 ∩ 本场 `roleMap`（需进战场） |
| cId | **无**（执行时用本场 roleMap 现算） | 有（实时） |
| 可选取 | 全员视为可选取（开战时按名册全量分配 cId，抓包证实） | 仅 roleMap 内成员 |
| 用途 | **编辑弹窗选队员、开战前的机动补齐** | 执行时实时校验、「候选」按钮预览实时排序/占用 |

排序规则两者一致（离线优先 → 战力降序，共用同一 sort 实现）。
...
### 3.6 执行模式：立即 / 等待（master 定稿 2026-09-16）

每支队伍一个 `mode` 字段（编辑弹窗里选，默认 `immediate`；旧数据无此字段按立即处理）。

两种模式的**共同第一步**：获取可组队角色清单 = `legion_getinfo` 名册 ∩ 本连接 `roleMap`（过程见 §2 ⓪~⑤）。

**① 立即模式（`immediate`，默认）** —— 拉一次清单，组完即走：

```
指定队员 ∉ 可组队清单（不在本俱乐部 roleMap）→ 直接放弃该角色（记 failed）
开启机动 → 从候选池按 §3.2 排序补人；无人可补 / 人数不够 → 不强制补满 5 人，有几个组几个
关闭机动 → 不补人
→ 全部邀请完成后立刻登场
```

**② 等待模式（`wait`）** —— 持续刷新清单，出现一个组一个：

```
持续获取可组队清单（轮询 war_getbattlefieldinfo 快照 + 广播增量帧）
指定队员「出现」一个 → 邀请一个
    「出现」= 可组队 = 未登场 且 未被任何角色组走（含未入队 + 入队后队伍未登场）
    ⇔ 战场状态 roles[cId].state === "watching"（teaming = 被组走，idle = 已登场）
    ⇔ getInviteReadiness()（legionWarState.js，纯函数，回归覆盖）
直到全部指定队员组上 → 再登场
一直等：没有超时；手动「停止」= 按当前已组队伍登场收尾
等待模式忽略「机动」开关（按 master 原话，机动只属于立即模式）
```

实现要点（`tasksSaltField.js`）：

- 每轮 `session.refreshBattlefieldInfo()` 主动拉快照 —— war_* 增量帧只在有变化时广播，被动收帧观测不到"被别人组走/自行登场"这类外部变化；快照响应 cmd 含 `war_getbattlefieldinfo`（与 `legionWarStore` 匹配方式一致）。
- 轮询间隔 `waitPollMs`（默认 12s，settings 可调）；邀请间隔沿用 `inviteIntervalMs`。
- 已在我队里的队员自动跳过（重跑友好）；邀请未确认的下一轮自动重试。
- ⚠️ 等待模式的队伍会**持续占用 1 个战场连接槽位直到收尾**——`maxActiveBattlefield`（默认 3）小于并发等待队伍数时，后面的队伍会排队等槽位。等待队伍多时先调大该设置。
- 校验的预期人数 = 1 + 已确认入队的指定队员数（等待中止时按实际到齐人数校验）。
...
"saltFieldAutoTeams": [
  { "id": "5143405-664483911", "legionId": 5143405,
    ...
    "mobile": true,                           // 机动：不满 5 人时按排序补齐（仅立即模式生效）
    "mode": "immediate" }                     // immediate=立即 | wait=等待（见 §3.6）；缺省按立即
],
