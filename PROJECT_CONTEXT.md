...
2026-09-07 真实 `wechat.bin` 登录复测：`wechat.bin` 大小为 1628 字节，readable 上号器已正确解码并安装 `LoginService.authUser` hook；官方 `authUser` 请求成功返回 `roleToken` 和 `roleId=119532574`。当前研究 bridge 已改为复用官方链：`PlatformManager.encryptUserInfo` -> `authorizeDeferred.resolve(info)` -> `LoginManager.login(true)`，不再依赖旧版 `triggerRelogin`。自动化浏览器页处于 `document.visibilityState=hidden` 时 Cocos 帧循环暂停，`_tryConnectServer` 记录 `0 connect failed`，因此该环境只能确认 auth 响应，不能把隐藏页测试误报为完整 WebSocket/角色登录成功；真实可见浏览器仍需确认 `NetworkManager.isConnected()`、`Role_GetRoleInfoResp` 和 `ROLE.authed`。

2026-09-08 首次登录时序修复验证：研究页“载入并登录”现在先持久化 BIN，再自动重启游戏 iframe；游戏入口等待 readable 上号器加载完成后才 `boot()`，模块就绪注入轮询缩短为 25ms。使用真实 `wechat.bin` 在可见浏览器完成一次干净验证：无需手动重启，页面收到 `account:login:complete`，`ROLE.authed=true`、`roleId=715582089`、`levelId=1`，Game 状态为 `GameRunning`。
...
- 2026-09-15 修正批量 Token 刷新日志：IP 限流期间保留原有批次槽位占用、无限等待和串行启动语义，不因限流释放槽位或跳过角色；批量页现在区分“开始调用刷新流程”“认证刷新限流器实际等待”“刷新调用实际返回的 success/retryable/reason”“调用异常”和“持续未返回状态”。
...
- [src/utils/batch/flexibleTemplate.js](src/utils/batch/flexibleTemplate.js)：自由模板的完整任务目录、参数规范化、持久化解析和共享连接协调器；当前目录共 58 个任务 ID（30 个 daily 任务、28 个批量 handler），重复入口通过旧 ID 别名归一化。
- 一键换皮闯关的 BOSS 目标通过 [src/utils/batch/skinChallengeUtils.js](src/utils/batch/skinChallengeUtils.js) 规范化；账号设置、旧任务模板和自由模板均支持选择 BOSS 1-6，空选表示跳过全部目标，周四补打也只检查已选择且未通关的目标。
...
批量日常主流程按 `maxActive` 分成账号波次：同一波账号并发执行，但下一波必须等待当前波全部结束后才启动。每个账号内部由 `DailyTaskRunner` 线性执行任务，日常任务完成后先逐项领取每日任务积分奖励，再领取每日完成奖励，随后处理周常和通行证奖励。连接失败时先由批量流程刷新 Token，再使用最新 Token 重连；刷新遇到限流或 HTTP 429 时每隔 1 秒重试直到成功。任务执行遇到限流、模块未开启、已知屏蔽或其他服务器错误时，都记录警告并只跳过当前任务，继续执行后续任务；挂机奖励和加钟流程中的 `400340` 统一视为限流，相关命令按 1 秒间隔重试，最多重试 100 次并记录当前重试次数；领取挂机奖励的 `system_claimhangupreward` 遇到请求超时也按同样策略重试。只有用户主动停止批处理或连接初始化失败才会结束当前账号。流程结束后在页面弹窗显示完成数量、总数量和最终失败角色清单，并可一键重新选中最终失败的账号。错误账号按最终状态汇总，不记录中间重试失败。

自由模板是与旧任务模板并存的第二套模板，不修改旧 `task-templates` 或账号 `daily-settings:*` 引用。自由模板当前分为完整日常、副本、宝库、怪异塔、资源、功法和月度与活动 7 个分组，共 58 个任务；日常批量分组已取消，重复的挂机、竞技场、俱乐部签到、黑市、珍宝阁和四国灯神入口统一保留完整日常任务，答题也放入完整日常组。资源批量入口包含宝箱周免费奖励：按顺序发送 `activity_buystoregoods { activityId: 7, goodsIndex: 0, buyNum: 1 }` 和 `activity_claimredquenchreward {}`，前者领取限时商店免费福利，后者领取红淬免费奖励；两步均通过 `SyncRewardResp` 的 `resp` 字段匹配。导入数据会过滤未知任务、把旧任务 ID 归一化到保留任务并规范化参数。执行时仍按 `maxActive` 分账号波次，同一波中的账号并发；同一模板内的多个任务同步启动，使任务自身的等待和冷却时间互相重叠。每个账号只占一个连接槽位并共享一个 WebSocket 生命周期，最慢任务结束后才统一断开；底层客户端继续通过独立发送队列和请求 `seq` 顺序发包、匹配响应。任一子任务失败只计入对应账号，不会把同一波其他账号误判为失败。每个任务的 UI 标签、实际命令、条件、循环和 `completed` 语义详见 [docs/flexible-batch-template-task-logic.md](docs/flexible-batch-template-task-logic.md)。
...
- [x] 已确认 A 类奖励逻辑：`1` 为累计战斗 3 次（不论胜负）；每个区域都有普通/困难/炼狱三档全清奖励。第一组全清抓包确认 `confId=5/6/7`，第二组确认 `8/9/10`，第三组确认 `11/12/13`；三组配置 ID 连续衔接，先前的 `2/3/4` 低概率备选已被第一组实测排除。
...
- [ ] 依据 [scripts/自动盐场.js](scripts/自动盐场.js)、[scripts/盐场攻击弹窗不消失(1).js](scripts/盐场攻击弹窗不消失(1).js) 和 [scripts/新锁头盐场阵容.js](scripts/新锁头盐场阵容.js)，继续分析盐场真实操作方法、调用顺序、前置状态和参数。
- [ ] 区分脚本中的真实 H5 模块调用、UI 修补和 sandbox/mock 分支，不能把 UI 行为或本地模拟当作协议证据。
- [ ] 根据用户反馈确认盐场是否可以正常上场和移动，但不能主动攻击其他玩家或建筑；分别验证“可上场”“可移动”“攻击玩家”“攻击建筑”四类动作及其失败原因。
- [ ] 记录上述动作对应的模块方法、底层命令、请求体、响应、错误码、连接类型和活动状态要求。

#### 4. 轻方案自动盐场

- [ ] 在重方案确认具体盐场指令、连接链和响应语义后，评估是否可以脱离游戏 runtime，直接通过 WebSocket/BON 协议自动执行盐场战斗。
- [ ] 轻方案必须先完成只读接口和低风险动作验证，再评估上场、移动、攻击、复活、组队等状态变更动作；未知协议不能直接批量发送。
- [ ] 对比 runtime 依赖、协议版本、战场专用连接、状态同步、限流和失败恢复成本，决定是否实施轻方案。

#### 5. 删除发车相关界面

- [x] 发车活动已经下线，已从当前项目 UI 删除发车相关按钮、配置项、状态展示和说明（2026-09-07）。
- [x] 已清理发车任务入口、自由模板任务、定时任务选项、车辆任务执行器和车辆协议注册；未改动旧副本目录 `xyzw_web_helper-main/`。
- [x] 已同步更新相关测试、文档和任务目录，旧模板或导入数据中的发车任务不再进入当前任务目录。

#### 6. 多账号登录游戏 runtime UI 与自动盐场控制
...
- [x] 批量运行时同步拆成「不同步（默认）/ 分组同步 / 全局同步」三种模式（`multiGameSyncMode`）：分组同步时每组只有组长能驱动本组其他窗口，全局同步时所有窗口跟随同一个同步源；组长默认取组内窗口顺序第一个，可手动指定或点窗口上的分组标签切换（`multiGameSyncMasters`）。父子页面用 send / receive 两个方向分别下发角色，从窗口不再上报自己的输入。
- [x] 全局同步源支持**点窗口标题直接指定**（`multiGameSyncGlobalSource`，全局模式生效、再点取消）：不依赖 Token 管理分组，临时拼的一批窗口没分组也能选队长；没手动指定（或该窗口已关）才回退到第一个分组的组长。
...
- [x] 批量页只维护「分组顺序 + 各组组长 + 同步模式 + 手动全局同步源」这四项本地状态；分组归属仍统一由 Token 管理维护，避免同一份数据两处编辑。
...
- [ ] 保留批量任务界面的“营地挑战”按钮，但完全改写其调度逻辑，不再按账号彼此独立地随机挑战。
- [ ] 以俱乐部为执行单位：先遍历选中的账号，读取所属俱乐部并建立俱乐部列表；账号只允许进入自己所属俱乐部的分组，且只处理本次选中的账号。
- [ ] 每个俱乐部内部，将选中的俱乐部账号按自身战力降序排列，并按该顺序依次安排挑战。
- [ ] 将对手角色按战力降序排列；每个进攻角色优先选择战力不高于 `自身战力 * threshold` 的敌人中战力最高者。
- [ ] 如果某次挑战战败，不重复挑战同一个打不过的敌人，改为选择符合条件的下一个敌人；需要记录已失败目标，避免在同一轮调度中回退重试。
- [ ] 每日总发起次数上限为 10 次；每日最多成功 3 次，普通玩家挑战和宠物挑战共享成功额度，达到 3 次成功后立即停止所有挑战。
- [ ] 当“剩余可发起挑战次数 == 剩余允许成功挑战次数”时，剩余次数全部攻击宠物进行保底；宠物视为敌方列表最后一个成员，目标选择必须避免因普通敌人全部不可战胜而浪费每日成功额度。
- [ ] 按已确认规则，宠物挑战为必胜保底目标；仍需在真实响应中确认宠物命令、计数更新和胜利结果字段。
- [ ] 一个俱乐部的全部选中成员完成战斗后，才统一执行该俱乐部的任务奖励领取；不得每个账号战斗结束就立即单独领奖。
- [ ] 每个俱乐部内部的挑战过程必须单线程执行，同一俱乐部不能同时让两个角色发起挑战；不同俱乐部是否可以并行，需要结合服务端限流和连接资源另行确认。
- [ ] 为俱乐部分组、战力排序、阈值选敌、失败换目标、宠物保底、10/3 次数边界、统一领奖和单线程调度增加 mock/集成测试。
...
截至 2026-09-07 的发车功能下线验证：

- 已从当前项目移除发车/收车 UI、车辆任务和车辆协议注册；旧嵌套副本 `xyzw_web_helper-main/` 未纳入本次清理。
- `node --test test/flexibleTemplate.test.js test/helperTaskRunner.test.js`：32 个用例全部通过。
