...
批量日常主流程和一键加钟按任务级并发槽位执行：一个账号从开始处理到成功或最终停止期间持续占用槽位，失败后每隔 1 秒重试，直到任务成功或用户停止批量执行；日常任务内部遇到服务器限流时会停留在当前角色的当前任务上，每隔 1 秒重试成功后才继续后续任务。流程结束后在页面弹窗显示完成数量、总数量和最终失败角色清单，并可一键重新选中最终失败的账号。错误账号按最终状态汇总，不记录中间重试失败。
